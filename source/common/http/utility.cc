...
void Utility::appendXff(HeaderMap& headers, const std::string& remote_address) {
  std::string forwarded_for = headers.ForwardedFor().value().c_str();
  ...
  headers.ForwardedFor().value(forwarded_for);
}

std::string Utility::createSslRedirectPath(const HeaderMap& headers) {
  ASSERT(headers.Host().present());
  ASSERT(headers.Path().present());
...
uint64_t Utility::getResponseStatus(const HeaderMap& headers) {
  ...
  if (!StringUtil::atoul(headers.Status().value().c_str(), response_code)) {
...
void Utility::sendLocalReply(StreamDecoderFilterCallbacks& callbacks, Code response_code,
  ...
  if (!body_text.empty()) {
    response_headers->ContentLength().value(body_text.size());
    response_headers->ContentType().value(Headers::get().ContentTypeValues.Text);
  }
...
std::string Utility::getLastAddressFromXFF(const Http::HeaderMap& request_headers) {
  if (!request_headers.ForwardedFor().present()) {
    return EMPTY_STRING;
  }
