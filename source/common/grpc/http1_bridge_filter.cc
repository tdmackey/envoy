...
Http::FilterHeadersStatus Http1BridgeFilter::decodeHeaders(Http::HeaderMap& headers, bool) {
  bool grpc_request = headers.ContentType().value() == Common::GRPC_CONTENT_TYPE.c_str();
  if (grpc_request) {
...
Http::FilterTrailersStatus Http1BridgeFilter::encodeTrailers(Http::HeaderMap& trailers) {
  ...
  if (do_bridging_) {
    ...
    if (grpc_status_header.present()) {
      ...
      response_headers_->GrpcStatus().value(grpc_status_header);
    }

    const Http::HeaderEntry& grpc_message_header = trailers.GrpcMessage();
    if (grpc_message_header.present()) {
      response_headers_->GrpcMessage().value(grpc_message_header);
    }
  }
